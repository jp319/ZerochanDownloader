...
@Composable
fun AppTheme(
    themePreference: String = "Orange",
    content: @Composable () -> Unit,
) {
    val (primaryColor, secondaryColor) =
        when (themePreference) {
            "Purple" -> Pair(DraculaPurple, DraculaPink)
            "Pink" -> Pair(DraculaPink, DraculaPurple)
            "Green" -> Pair(DraculaGreen, DraculaCyan)
            "Red" -> Pair(DraculaRed, DraculaOrange) // Red is usually error, but can be a theme
            "Yellow" -> Pair(DraculaYellow, DraculaOrange)
            "Cyan" -> Pair(DraculaCyan, DraculaGreen)
            else -> Pair(DraculaBurntOrange, DraculaOrange) // "Orange"
        }

    val dynamicColorScheme =
        DraculaDarkColorScheme.copy(
